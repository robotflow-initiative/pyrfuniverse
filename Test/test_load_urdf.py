...
env.instance_channel.set_action(
    ...
    id=robot_id,
    quaternion=tool.UnityEularToQuaternion([0, 90, 0]),
    duration=30,
    relative=True
)
env._step()
while not env.instance_channel.data[robot_id]['move_done'] or not env.instance_channel.data[robot_id]['rotate_done']:
    env._step()
