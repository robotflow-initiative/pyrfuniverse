...
class RFUniverseStretchController:
    ...
    def __init__(
            ...
            base_orn=[-0.707107, 0.0, 0.0, 0.707107], init_joint_positions=[0] * 12,
            render=False
    ):
        if render:
        ...
        self.num_dof = 12
        self.init_joint_positions = self.get_pybullet_joint_pos_from_unity(init_joint_positions)
        self.robot = self.bullet_client.loadURDF(self.robot_urdf, base_pos, base_orn, useFixedBase=True,
        ...
        for j in range(36):
            ...
            jointName = info[1]
            jointType = info[2]
            if jointType == self.bullet_client.JOINT_REVOLUTE or jointType == self.bullet_client.JOINT_PRISMATIC:
                # print(j, jointName)
    ...
    def get_unity_joint_pos_from_pybullet(self, pybullet_joint_pos: tuple) -> list:
        ...
        unity_joint_pos = list(pybullet_joint_pos[10:12]) + list(pybullet_joint_pos[2:8]) + \
    ...
    def calculate_ik(self, unity_eef_pos, eef_orn=None) -> list:
        if eef_orn is None:
            eef_orn = self.bullet_client.getQuaternionFromEuler([math.pi / 2., 0., 0.])
    ...
    def reset(self):
        for i, (idx) in enumerate(self.moveable_joint_ids):
            self.bullet_client.resetJointState(self.robot, idx, self.init_joint_positions[i])
