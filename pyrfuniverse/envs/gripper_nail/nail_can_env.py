...
class NailCanEnv(RFUniverseGymWrapper):
    def __init__(
            self,
    ...
    def reset(self):
        ...
        self.articulation_channel.set_action(
            ...
            index=0,
            joint_positions=[-self.radius * math.cos(theta), 0, -self.radius * math.sin(theta), -45]
        )
        self._step()
    ...
    def _get_obs(self):
        gripper_position = np.array(self.articulation_channel.data[0]['positions'][9])
        gripper_velocity = np.array(self.articulation_channel.data[0]['velocities'][9])
        ...
        ring_rotation = np.array([self.articulation_channel.data[1]['joint_positions'][0]])

        obs = np.concatenate((
            gripper_position, gripper_velocity, gripper_extra_param,
            ring_pos, ring_rotation
        ))
